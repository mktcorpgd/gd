...
jQuery(document).ready(function() {
	...
	if ( jQuery('.address_maps').length ) {
		...
		addressAutocomplete2.addListener('place_changed', function() {
			...
			jQuery('input[name=IDATO]').after('<img src="https://maps.googleapis.com/maps/api/staticmap?size=300x300&maptype=roadmap&markers=size:mid%7Ccolor:red%7C'+address+'&zoom=16&key=AIzaSyBgKiaX5D3Pp4Jx16S_JBH4_vUJngmX3PM" alt="Hasta" />');
		});
		addressAutocomplete3.addListener('place_changed', function() {
			var address = addressAutocomplete3.getPlace().formatted_address;
			jQuery('input[name=VUELTAFROM]').addClass('filledwithmap').after('<img src="https://maps.googleapis.com/maps/api/staticmap?size=300x300&maptype=roadmap&markers=size:mid%7Ccolor:red%7C'+address+'&zoom=16&key=AIzaSyBgKiaX5D3Pp4Jx16S_JBH4_vUJngmX3PM" alt="'+address+'" />');
		});
