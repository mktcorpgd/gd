...
jQuery(document).ready(function() {
	...
	jQuery(document).on('click','#prensa .fusion-post-content.post-content>h2>a,#prensa span.meta-tags a,.tribe-events-calendar-list__event.tag-redirect a',function(e) {
	...
	jQuery(':input[type=text],:input[type=email],textarea,select').each(function(i){
		if ( jQuery(this).val().length > 0 ) {
			jQuery(this).parents('.fusion-layout-column').addClass('focused');
		};
	});
	jQuery(document).on('focus',':input[type=text],:input[type=email],textarea,select',function(){
		jQuery(this).parents('.fusion-layout-column').addClass('focused');
	});
	jQuery(document).on('blur',':input[type=text],:input[type=email],textarea,select',function(){
